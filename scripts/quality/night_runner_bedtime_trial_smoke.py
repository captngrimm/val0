...
def test_report_written_and_safe(tmpdir: Path) -> None:
    ...
    assert_contains(report, "PASS_BEDTIME_TRIAL_READY", "report decision")
    assert_contains(report, "Night Runner Bedtime Report", "polished report title")
    assert_contains(report, "Decision", "decision section")
    assert_contains(report, "Task", "task section")
    assert_contains(report, "Tests Run", "tests run section")
    assert_contains(report, "Test Results", "test results section")
    assert_contains(report, "Changed Files", "changed files section")
    assert_contains(report, "Safety Status", "safety status section")
    assert_contains(report, "Protected Live Data", "protected live data section")
    assert_contains(report, "Morning Review Options", "morning review options section")
    assert_contains(report, "Recommended Next Step", "recommended next step section")
    assert_contains(report, "exit 0", "exit code captured")
    assert_contains(report, "report_path:", "report path line")
    assert_contains(report, "NIGHT-RUNNER-20", "next lane")
