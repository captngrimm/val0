...
def _assert_recommendation(reply: str, expected_source: str, label: str) -> None:
    ...
    assert_contains(reply, "Mi recomendación: empezamos con el flujo Organizar mi día", f"{label} recommends daily flow")
    assert_contains(reply, "Semana 1 sería simple", f"{label} week 1 plan")
    assert_contains(reply, "Identificar dónde entran tus pendientes", f"{label} gather sources")
    assert_contains(reply, "Separar agenda, tareas y recordatorios", f"{label} separates categories")
    assert_contains(reply, "revisión diaria corta", f"{label} daily review")
    assert_contains(reply, "solo después de confirmarlo", f"{label} confirmation before reminders/tasks")
    ...
    assert_contains(reply, "¿Quieres que usemos este como tu primer flujo piloto?", f"{label} asks confirmation")
