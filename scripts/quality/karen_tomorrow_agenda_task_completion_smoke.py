...
def test_tomorrow_agenda_copy() -> None:
    ...
    assert_contains(dashboard, "📌 Recordatorios y tareas", "agenda has simplified internal heading")
    assert_not_contains(dashboard, "Agenda interna de Val", "agenda removes confusing internal label")
    assert_contains(tomorrow, "⏰ Recordatorios", "tomorrow keeps reminders section")
    assert_contains(tomorrow, "📌 Tareas", "tomorrow keeps tasks section")
