...
def build_readonly_plan_prompt(packet: dict[str, Any]) -> str:
    operator_prompt = str(packet.get("task_prompt") or "").strip()
    return "\n".join(
        [
            "Night Runner read-only planning report.",
            "",
            "You are running in a guarded read-only planning invocation.",
            "Do not edit files.",
            ...
            "You may inspect repo context read-only and output a short implementation plan.",
            "Output only a plan/report; do not create patches.",
            "",
            f"Planning prompt: {operator_prompt}",
        ]
    )


def _last_message_path(packet: dict[str, Any]) -> Path:
    report = Path(str(packet.get("report_path") or "tmp/night_runner/codex_readonly_plan_report.md"))
    stem = report.name.rsplit(".", 1)[0] if report.name else "codex_readonly_plan_report"
    return ROOT / "tmp" / "night_runner" / f"{stem}_last_message.txt"
...
def build_codex_command(codex_path: str, packet: dict[str, Any]) -> tuple[str, ...]:
    return (
        ...
        "--ephemeral",
        "-o",
        str(_last_message_path(packet)),
        "--",
        build_readonly_plan_prompt(packet),
    )
...
def _validate_packet(
    ...
    *,
    ...
    if str(packet.get("task_mode") or "").strip() != "readonly_plan":
        reasons.append("task_mode must be readonly_plan")
    if require_execute and packet.get("allow_codex_execute") is not True:
        reasons.append("allow_codex_execute must be true for actual read-only planning invocation")
    ...
    if not reasons:
        return "PASS_READONLY_PLAN_INVOKE_READY" if require_execute else "PASS_READONLY_PLAN_DRY_RUN_READY", ()
...
def render_report(
    ...
    *,
    ...
    lines = [
        "Night Runner Codex Read-only Planning Report",
        "============================================",
        "",
...
def evaluate_packet(
    ...
    *,
    ...
    if invoke and decision == "PASS_READONLY_PLAN_INVOKE_READY":
        last_message = _last_message_path(packet)
    ...
    if executed and codex_exit_code not in {0}:
        ...
        decision = "CODEX_READONLY_PLAN_FAILED"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run one guarded Codex read-only planning invocation from a packet.")
    parser.add_argument("--packet", required=True, type=Path, help="Path to read-only planning packet.")
    parser.add_argument("--codex-path", help="Optional explicit Codex binary path override.")
    parser.add_argument("--dry-run-only", action="store_true", help="Validate and build the command, but do not invoke Codex.")
    args = parser.parse_args(argv)

    try:
        packet = load_lane_packet(args.packet)
    except Exception as exc:
        print("Night Runner Codex Read-only Planning Report")
        print("============================================")
        print("")
    ...
    if result.decision in {"PASS_READONLY_PLAN_INVOKE_READY", "PASS_READONLY_PLAN_DRY_RUN_READY"}:
        return 0
    if result.decision == "CODEX_READONLY_PLAN_FAILED":
        return 1
    return 2
