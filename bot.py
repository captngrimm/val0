...
from core.case_timeline_events import maybe_handle_case_timeline_event_confirmation, maybe_handle_case_timeline_event_draft
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if await maybe_handle_onboarding_discovery(update, context, chat_id, client_id, text):
            _maybe_log_intent_router_v2_actual("onboarding_discovery", "maybe_handle_onboarding_discovery", chat_id=chat_id, message_id=tg_msg_id, text=text)
            return
    except Exception as e:
        logger.exception(f"[ONBOARDING_DISCOVERY_HANDLE_TEXT] failed: {e}")
    ...
    try:
        if await maybe_handle_case_timeline_event_confirmation(update, context, chat_id, client_id, text):
            _maybe_log_intent_router_v2_actual("destructive_confirmation", "maybe_handle_case_timeline_event_confirmation", chat_id=chat_id, message_id=tg_msg_id, text=text)
            return
    except Exception as e:
        logger.exception(f"[KAREN_CASE_TIMELINE_EVENT_CONFIRMATION_HANDLE_TEXT] failed: {e}")
    ...
    try:
        if await maybe_handle_case_timeline_event_draft(update, context, chat_id, client_id, text):
            _maybe_log_intent_router_v2_actual("case_status", "maybe_handle_case_timeline_event_draft", chat_id=chat_id, message_id=tg_msg_id, text=text)
            return
    except Exception as e:
        logger.exception(f"[KAREN_CASE_TIMELINE_EVENT_DRAFT_HANDLE_TEXT] failed: {e}")
