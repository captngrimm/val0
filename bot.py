...
# --------------------------------------------------
# Attachment inventory — case-linked VFMS uploads v0
# --------------------------------------------------
def _attachment_note_field(note_text: str, label: str) -> str:
    prefix = f"- {label}:"
...
async def maybe_handle_attachment_inventory(update: Update, context: ContextTypes.DEFAULT_TYPE, chat_id: int, text: str) -> bool:
    """
    Answers:
    ...
    using case_notes generated by telegram_attachment_vfms.
    """
    raw = (text or "").strip()
    norm = unicodedata.normalize("NFKD", raw.lower())
    norm = "".join(c for c in norm if not unicodedata.combining(c))

    wants_docs = any(x in norm for x in [
    ...
    if not case_id:
        await update.message.reply_text(
            "Todavía no tengo un caso activo para este chat. Primero hay que activar o registrar el caso."
        )
        return True
    ...
    try:
        ...
        logger.exception(f"Attachment inventory query failed: {e}")
        await update.message.reply_text("No pude leer el inventario de documentos ahora mismo.")
        return True

    if not rows:
        await update.message.reply_text(
            f"No veo documentos registrados todavía para CASE:{case_id}."
        )
        return True
    ...
    for row in rows:
        try:
            ...
            note_id, created_at, note_text = row[0], row[1], row[2]

        filename = _attachment_note_field(note_text, "Archivo") or "archivo sin nombre"
        ingest_id = _attachment_note_field(note_text, "VFMS ingest_id")
        user_note = _attachment_note_field(note_text, "Nota usuario")
        status = _attachment_note_field(note_text, "Estado")
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --------------------------------------------------
    try:
        if await maybe_handle_attachment_inventory(update, context, chat_id, text):
            return
