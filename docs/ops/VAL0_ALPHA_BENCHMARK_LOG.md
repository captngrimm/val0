...
NIGHT-RUNNER-17 is sealed. Recommended next: ValPrime checkpoint, then NIGHT-RUNNER-18 Manual Bedtime Trial. This should run the bedtime workflow manually end-to-end and verify report output; still no runtime bot/core behavior, no client data, no restarts, no live DB writes, no commits unless explicitly approved.
