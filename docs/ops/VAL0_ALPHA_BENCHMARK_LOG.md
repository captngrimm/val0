...
NIGHT-RUNNER-08 is sealed. Recommended next: ValPrime checkpoint, then NIGHT-RUNNER-09 Branch-only Documentation Diagnostic. This is the first controlled file-edit lane: docs-only, branch-only, no runtime behavior, no client data, no commits unless explicitly approved later.
