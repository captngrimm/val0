...
NIGHT-RUNNER-09 is sealed. Recommended next: ValPrime checkpoint, then NIGHT-RUNNER-10 Branch-only Tiny Docs Patch With Smoke. This allows one docs-only edit plus a focused smoke; still no runtime behavior, no client data, no commits unless explicitly approved.
