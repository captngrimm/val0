...
| Ask about contradiction / weirdness | `Val, hay algo raro o contradictorio en el Caso Finca?` | `Val, ves algo raro?`; `Val, algo no cuadra?` with active case context | Contradiction/weirdness Q&A answer, framed as items to review | `scripts/quality/caso_finca_conversational_qa_smoke.py` protects explicit phrase and context-gated vague variants | Keep generic no-context weirdness phrases from routing to Caso Finca | Frame as “cosas que revisaría con Nora”; no conclusions | PASS with active/explicit Caso Finca context |
